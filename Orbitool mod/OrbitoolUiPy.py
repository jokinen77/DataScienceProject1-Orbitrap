...
#imports for bad debugging with dialogs. Remove without worry (if proper debug is used)
...
class Window(QtWidgets.QMainWindow, OrbitoolUi.Ui_MainWindow):
    ...
    def clear(self, beginFrom=0):
        ...
        if beginFrom <= 1:
            ...
            workspace.noiseParam = None
            workspace.denoisedSpectrum1 = None
            workspace.spectra2Noiseparams = None
    ...
    def workspaceImportFinished(self, result, args):
        ...
        self.showSpectrum1(workspace.spectrum1,
                           workspace.noiseParam, workspace.denoisedSpectrum1)
    ...
    def qSpectra1Show(self, index):
        ...
        def process(sendStatus):
            ...
            peakAt, noise, noiseParam = OrbitoolFunc.getNoise(
                spectrum, quantile, sendStatus)
            denoisedSpectrum = OrbitoolFunc.denoiseWithLOD(
                spectrum, noiseParam, peakAt, minus, sendStatus)
            return (spectrum, noiseParam, denoisedSpectrum)
    ...
    def qSpectra1ShowFinished(self, result, args):
        spectrum, noiseParam, denoisedSpectrum1 = result
        workspace = self.workspace
        workspace.spectrum1 = spectrum
        workspace.noiseParam = noiseParam
        workspace.denoisedSpectrum1 = denoisedSpectrum1

        self.showSpectrum1(spectrum, noiseParam, denoisedSpectrum1)

    @restore
    def showSpectrum1(self, spectrum: OrbitoolBase.Spectrum, noiseParam: np.ndarray, denoisedSpectrum1: OrbitoolBase.Spectrum):
        # calculate noise
        noise, LOD = OrbitoolFunc.calNoise(spectrum.mz, noiseParam)
        ...
        ax.plot(spectrum.mz, LOD, color='r', label='LOD')
        ax.plot(spectrum.mz, noise, color='r', label='noise', linestyle='--')
    ...
    def qDenoiseRecalcFinished(self, result, args):
        noise, noiseParam, denoisedSpectrum1 = result
        workspace = self.workspace
        workspace.noise = noise
        workspace.noiseParam = noiseParam
        workspace.denoisedSpectrum1 = denoisedSpectrum1
        self.showSpectrum1(workspace.spectrum1, noiseParam, denoisedSpectrum1)
    ...
    def qDenoise(self, withDenoising=True):
        ...
        def process(sendStatus):
            ...
            with multiprocessing.Pool(cpu) as pool:
                if not withDenoising:
                    ...
                    for index, ret in enumerate(rets):
                        ...
                        noise, noiseParam, spectrum = ret.get()
                        noiseParams.append(noiseParam)
                        spectra.append(spectrum)
                        maps.setdefault(spectrum.fileTime,
                                        []).append(spectrum)
                if len(operators) > 0:
            ...
            return noiseParams, spectra, maps, newoperators
        thread = QThread(process, tuple())
    ...
    Single threaded function (no UI update) for processing spectra.
    '''
    def qSpectra1Process(self, index):
        minus = self.denoiseRemoveMinusRadioButton.isChecked()
        workspace = self.workspace
        fileList = self.fileList
        spectrum = None
        ...
        peakAt, noise, noiseParam = OrbitoolFunc.getNoise(
            spectrum, quantile)
        denoisedSpectrum = OrbitoolFunc.denoiseWithLOD(
            spectrum, noiseParam, peakAt, minus)
        return denoisedSpectrum

    '''
    Custom addition from original software.
    Export denoised spectra function for all the spectra.
    '''
    @threadBegin
    @withoutArgs
    @openfile('Select a folder to save', folder=True)
    ...
    def qDenoiseFinished(self, result, args):
        noiseParams, spectra, maps, operators = result
        workspace = self.workspace
        workspace.spectra2noiseParams = noiseParams
